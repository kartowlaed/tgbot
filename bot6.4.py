...
def get_main_menu_markup(user_id):
    ...
    btn_notify = types.InlineKeyboardButton(
        "🔊 Уведомления" if user.get("subscribed", False) else "🔇 Уведомления",
...
def admin_roles_menu(call):
    ...
    # Фиксированный набор ролей с кодами
    roles_list = {
        "PRES001": "Президент",
        "MAY002": "Мэр",
    ...
    for code, role_name in roles_list.items():
        owner = None
        # Ищем пользователя, у которого поле "role" совпадает с role_name
        for uid, user in users.items():
            if user.get("role") == role_name:
                nickname = user.get("nickname", "—")
                username = user.get("telegram_username", "")
                # Для "Президент" выводим имя без ссылки
                ...
                break
        if owner:
            text += f"{code}: <b>{role_name}</b> — {owner}\n"
        else:
            text += f"{code}: <b>{role_name}</b> — <i>не назначено</i>\n"
...
@bot.callback_query_handler(func=lambda call: call.data == "admin_update")
def admin_update_prompt(call):
    if call.from_user.id != ADMIN_ID:
        bot.answer_callback_query(call.id, "Нет доступа.")
        return
    msg = bot.send_message(
        call.message.chat.id,
        "Введите текст обновления или отправьте фото с подписью:"
    )
...
def community_menu(call):
    # Разметка с двумя кнопками в ряду
    markup = types.InlineKeyboardMarkup(row_width=2)
    ...
    # Отдельно — кнопка возврата
    markup.add(btn_back)

    bot.edit_message_text(
        "📌 *Сообщество* — выбери, что хочешь посмотреть:",
        call.message.chat.id,
        call.message.message_id,
        parse_mode="Markdown",
        reply_markup=markup
...
def handle_tribes_page_safe(call):
    ...
    data = load_data()
    tribes = list(data.get("tribes", {}).values())
    # Перемешиваем порядок, чтобы при каждом обновлении он был разным
    random.shuffle(tribes)
...
def handle_daily_gift(call):
    ...
    # Если уже брал подарок сегодня — просто сообщаем
    if user.get("last_daily_gift") == today:
        ...
        return bot.answer_callback_query(call.id, f"Приходите через {hrs}ч{mins}м.")

    # Выдача награды (тот же код, что был)
    rnd = random.random()
    if rnd < 0.01:
        # Джекпот
        if random.choice([True, False]):
            amt = 10; user["balance"] = user.get("balance",0) + amt
            text = f"Jackpot! +{amt}💰"
        else:
            amt = 10; user["ender_eyes"] = user.get("ender_eyes",0) + amt
            text = f"Jackpot! +{amt}🧿"
    elif rnd < 0.10:
        eyes = random.randint(1,5); coins = random.randint(1,5)
        user["ender_eyes"] += eyes; user["balance"] += coins
        text = f"Комбоподарок! 🧿×{eyes}, 💰×{coins}"
    elif rnd < 0.30:
        coins = random.randint(1,5); user["balance"] += coins
        text = f"+{coins}💰 монет"
    else:
        eyes = random.randint(1,5); user["ender_eyes"] += eyes
        text = f"+{eyes}🧿 Эндера"
    ...
    # Отвечаем и обновляем меню
    bot.answer_callback_query(call.id, text)
    bot.send_message(call.message.chat.id, f"🎁 {text}")

    # Перерисовываем кнопку подарка
    new_kb = market_main_markup(user_id)
    try:
...
def show_roles(call):
    ...
    for _, role_name in roles_list.items():  # не используем код в выводе
        owner = None
        for uid, user in users.items():
            if user.get("role") == role_name:
                nickname = user.get("nickname", "—")
...
def safe_send_message(chat_id, text, **kwargs):
    try:
        ...
        if e.error_code == 429:
            retry = int(re.search(r"retry after (\d+)", str(e)).group(1))
            time.sleep(retry + 1)
            return bot.send_message(chat_id, text, **kwargs)
        else:
            raise
