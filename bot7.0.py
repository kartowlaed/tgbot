...
def get_main_menu_markup(user_id):
    ...
    # ── Ряд 4: Подать дело в суд
    btn_court = types.InlineKeyboardButton("⚖️ Подать в суд", callback_data="court_start")
    markup.row(btn_court)

    if user.get("role") in ["Президент", "Прокурор", "Мэр"]:
        markup.row(types.InlineKeyboardButton("📂 Судебные дела", callback_data="court_list"))
...
def community_menu(call):
    ...
    # базовые кнопки «Трайбы», «Роли», «Игроки», «Топ стриков»
    ...
    btn_top_streaks = types.InlineKeyboardButton("🔥 Топ стриков", callback_data="top_streaks")

    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(btn_tribes, btn_roles, btn_players, btn_top_streaks)
...
def show_top_streaks(call):
    ...
    back_btn = types.InlineKeyboardButton("🔙 Назад", callback_data="community_menu")
...
@bot.callback_query_handler(func=lambda c: c.data == "court_start")
def court_start(call):
    user_id = str(call.from_user.id)
    user_states[user_id] = {"state": "court_title", "temp_data": {}}
    bot.send_message(call.message.chat.id, "Введите название дела:")


@bot.message_handler(func=lambda m: str(m.from_user.id) in user_states and user_states[str(m.from_user.id)]["state"] == "court_title")
def court_title(message):
    uid = str(message.from_user.id)
    user_states[uid]["temp_data"]["title"] = message.text
    user_states[uid]["state"] = "court_short"
    bot.send_message(message.chat.id, "Краткое описание дела:")


@bot.message_handler(func=lambda m: str(m.from_user.id) in user_states and user_states[str(m.from_user.id)]["state"] == "court_short")
def court_short(message):
    uid = str(message.from_user.id)
    user_states[uid]["temp_data"]["short_desc"] = message.text
    user_states[uid]["state"] = "court_desc"
    bot.send_message(message.chat.id, "Полное описание дела:")


@bot.message_handler(func=lambda m: str(m.from_user.id) in user_states and user_states[str(m.from_user.id)]["state"] == "court_desc")
def court_desc(message):
    uid = str(message.from_user.id)
    user_states[uid]["temp_data"]["description"] = message.text
    user_states[uid]["state"] = "court_defendant"
    bot.send_message(message.chat.id, "Укажите @username ответчика или напишите 'Неизвестен':")


@bot.message_handler(func=lambda m: str(m.from_user.id) in user_states and user_states[str(m.from_user.id)]["state"] == "court_defendant")
def court_defendant(message):
    uid = str(message.from_user.id)
    data = load_data()
    user_states[uid]["temp_data"]["defendant"] = message.text
    user_states[uid]["temp_data"]["defendant_id"] = find_user_by_nick_or_username(message.text, data)
    user_states[uid]["state"] = "court_screens"
    bot.send_message(message.chat.id, "Пришлите до 4 скриншотов одним сообщением:")


@bot.message_handler(content_types=["photo"], func=lambda m: str(m.from_user.id) in user_states and user_states[str(m.from_user.id)]["state"] == "court_screens")
def court_screens(message):
    uid = str(message.from_user.id)
    photos = [p.file_id for p in message.photo]
    user_states[uid]["temp_data"]["screenshots"] = photos[:4]
    user_states[uid]["state"] = "court_comp"
    bot.send_message(message.chat.id, "Какую компенсацию вы требуете?")


@bot.message_handler(func=lambda m: str(m.from_user.id) in user_states and user_states[str(m.from_user.id)]["state"] == "court_comp")
def court_compensation(message):
    uid = str(message.from_user.id)
    user_states[uid]["temp_data"]["compensation"] = message.text
    temp = user_states[uid]["temp_data"]
    summary = (
        f"<b>Название:</b> {temp['title']}\n"
        f"<b>Кратко:</b> {temp['short_desc']}\n"
        f"<b>Описание:</b> {temp['description']}\n"
        f"<b>Ответчик:</b> {temp['defendant']}\n"
        f"<b>Компенсация:</b> {temp['compensation']}"
    )
    markup = types.InlineKeyboardMarkup()
    markup.add(
        types.InlineKeyboardButton("Создать дело", callback_data="court_confirm"),
...
@bot.callback_query_handler(func=lambda c: c.data == "court_confirm")
def court_confirm(call):
    uid = str(call.from_user.id)
    temp = user_states.get(uid, {}).get("temp_data")
    if not temp:
        bot.answer_callback_query(call.id)
        return
    data = load_data()
    case_id = data.get("next_case_id", 1)
    data["next_case_id"] = case_id + 1
    case = {
        "id": case_id,
        "plaintiff_id": uid,
        "title": temp.get("title"),
        "short_desc": temp.get("short_desc"),
        "description": temp.get("description"),
        "defendant": temp.get("defendant"),
        "defendant_id": temp.get("defendant_id"),
        "screenshots": temp.get("screenshots", []),
        "compensation": temp.get("compensation"),
        "status": "open",
        "verdict": "",
        "created_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    }
    data.setdefault("court_cases", []).append(case)
    save_data(data)
    if case.get("defendant_id"):
        try:
            bot.send_message(int(case["defendant_id"]), f"На вас подали в суд. Дело #{case_id}.")
        except Exception:
            pass
    user_states.pop(uid, None)
    bot.edit_message_text(
        f"Дело #{case_id} создано.",
        call.message.chat.id,
        call.message.message_id,
        reply_markup=get_main_menu_markup(uid)
    )


@bot.callback_query_handler(func=lambda c: c.data == "court_list")
def court_list(call):
    uid = str(call.from_user.id)
    data = load_data()
    role = data["users"].get(uid, {}).get("role")
    if role not in ["Президент", "Прокурор", "Мэр"]:
        bot.answer_callback_query(call.id, "Нет доступа")
        return
    cases = [c for c in data.get("court_cases", []) if c.get("status") in ["open", "in_progress"]]
    text = "<b>Активные дела:</b>" if cases else "Нет активных дел"
    markup = types.InlineKeyboardMarkup()
    for c in cases:
        markup.add(types.InlineKeyboardButton(f"Дело {c['id']}", callback_data=f"court_view_{c['id']}"))
    markup.add(types.InlineKeyboardButton("🔙 Главное меню", callback_data="get_main_menu_markup"))
    bot.edit_message_text(text, call.message.chat.id, call.message.message_id, parse_mode="HTML", reply_markup=markup)


@bot.callback_query_handler(func=lambda c: c.data.startswith("court_view_"))
def court_view(call):
    uid = str(call.from_user.id)
    data = load_data()
    role = data["users"].get(uid, {}).get("role")
    ...
    case = next((c for c in data.get("court_cases", []) if c["id"] == case_id), None)
    if not case:
    ...
    text = (
        ...
        f"Кратко: {case['short_desc']}\n"
        f"Описание: {case['description']}\n"
        f"Ответчик: {case['defendant']}\n"
        f"Требования: {case['compensation']}\n"
        f"Статус: {case['status']}"
    )
    if case.get('status') == 'closed' and case.get('verdict'):
        text += f"\nВердикт: {case['verdict']}"
    if case.get('status') == 'rejected' and case.get('reject_reason'):
        text += f"\nПричина отказа: {case['reject_reason']}"
    markup = types.InlineKeyboardMarkup()
    if role in ["Президент", "Прокурор"]:
        if case["status"] == "open":
            markup.add(
                types.InlineKeyboardButton("Взять в работу", callback_data=f"court_take_{case_id}"),
                types.InlineKeyboardButton("Отклонить", callback_data=f"court_reject_{case_id}")
            )
        elif case["status"] == "in_progress" and case.get("assigned_to") == uid:
            markup.add(types.InlineKeyboardButton("Вынести вердикт", callback_data=f"court_verdict_{case_id}"))
    markup.add(types.InlineKeyboardButton("◀️ Назад", callback_data="court_list"))
    if case.get("screenshots"):
        media = [InputMediaPhoto(p) for p in case["screenshots"]]
        try:
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("court_take_"))
def court_take(call):
    uid = str(call.from_user.id)
    case_id = int(call.data.split("_")[2])
    data = load_data()
    role = data["users"].get(uid, {}).get("role")
    if role not in ["Президент", "Прокурор"]:
        bot.answer_callback_query(call.id, "Нет доступа")
        return
    case = next((c for c in data.get("court_cases", []) if c["id"] == case_id), None)
    if not case:
    ...
    save_data(data)
    bot.edit_message_text("Дело взято в работу.", call.message.chat.id, call.message.message_id)


@bot.callback_query_handler(func=lambda c: c.data.startswith("court_verdict_"))
def court_verdict_start(call):
    uid = str(call.from_user.id)
    case_id = int(call.data.split("_")[2])
    user_states[uid] = {"state": "court_verdict_text", "temp_data": {"case_id": case_id}}
    bot.send_message(call.message.chat.id, "Введите текст вердикта:")


@bot.message_handler(func=lambda m: str(m.from_user.id) in user_states and user_states[str(m.from_user.id)]["state"] == "court_verdict_text")
def court_verdict_save(message):
    uid = str(message.from_user.id)
    verdict = message.text
    data = load_data()
    case_id = user_states[uid]["temp_data"]["case_id"]
    case = next((c for c in data.get("court_cases", []) if c["id"] == case_id), None)
    if not case:
        user_states.pop(uid, None)
        return
    ...
    case["status"] = "closed"
    case["closed_at"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    save_data(data)
    plaintiff = case.get("plaintiff_id")
    if plaintiff:
        try:
            bot.send_message(int(plaintiff), f"Ваше дело #{case_id} рассмотрено. Вердикт:\n{verdict}")
        except Exception:
            pass
    if case.get("defendant_id"):
        try:
            bot.send_message(int(case["defendant_id"]), f"Суд вынес вердикт по делу #{case_id}.\n{verdict}")
        except Exception:
            pass
    user_states.pop(uid, None)
    bot.send_message(message.chat.id, f"Вердикт по делу #{case_id} сохранён.", reply_markup=get_main_menu_markup(uid))


@bot.callback_query_handler(func=lambda c: c.data.startswith("court_reject_"))
def court_reject_start(call):
    uid = str(call.from_user.id)
    case_id = int(call.data.split("_")[2])
    data = load_data()
    role = data["users"].get(uid, {}).get("role")
    ...
    if not case:
        bot.answer_callback_query(call.id, "Не найдено")
        return
    user_states[uid] = {"state": "court_reject_reason", "temp_data": {"case_id": case_id}}
    bot.send_message(call.message.chat.id, "Введите причину отклонения дела:")


@bot.message_handler(func=lambda m: str(m.from_user.id) in user_states and user_states[str(m.from_user.id)]["state"] == "court_reject_reason")
def court_reject_reason(message):
    uid = str(message.from_user.id)
    reason = message.text
    data = load_data()
    case_id = user_states[uid]["temp_data"]["case_id"]
    ...
    if plaintiff:
        try:
            bot.send_message(int(plaintiff), f"Ваше дело #{case_id} отклонено. Причина: {reason}")
        except Exception:
            pass
    user_states.pop(uid, None)
    bot.send_message(message.chat.id, "Дело отклонено.", reply_markup=get_main_menu_markup(uid))
